...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($reported_id <= 0) $errors[] = 'Select a user.';
    if($reason === '') $errors[] = 'Reason is required.';

    if(!$errors){
        $check = $connection->prepare("SELECT 1 FROM blacklists WHERE reporter_id=? AND reported_id=?");
        $check->bind_param('ii', $user_id, $reported_id);
        $check->execute();
        $check->store_result();
        if($check->num_rows > 0) $errors[] = "You already reported this user.";
        $check->close();
    }

    if(!$errors){
        $stmt = $connection->prepare("INSERT INTO blacklists (reporter_id, reported_id, reason) VALUES (?,?,?)");
    ...
    if(!$errors){
        $cntStmt = $connection->prepare("SELECT COUNT(*) AS cnt FROM blacklists WHERE reported_id=?");
        ...
        if($cnt >= 4){
            $stmt = $connection->prepare("UPDATE users SET is_active=0 WHERE id=? AND role!='admin'");
...
<div class="container">
    <div class="card">
        <h2>Report User</h2>
        <p><a href="<?= htmlspecialchars($backLink) ?>" class="button">Back to Dashboard</a></p>
        ...
        <form method="post">
            <label for="reported_id">Select User</label>
            <select name="reported_id" id="reported_id" required>
